...
import { products as allProducts, getColorClass } from "@/lib/data"
import type { Product } from "@/lib/data"
import Link from "next/link"

const featuredProducts = allProducts.filter((p) => p.isFeatured && p.status !== "draft")
                  ...
                  <Link href={`/products/${product.id}`}>
                    <div className="relative aspect-square overflow-hidden">
                      ...
                      <div className="absolute bottom-3 left-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <Button className="w-full" size="sm" onClick={(e) => { e.preventDefault(); setQuickViewProduct(product) }}>
